...
*2015.07.17*: added a new =hcapi()= method that combines =getLayer()=, =genFile()=, and =getLayerWKT()=. =genFile()= no longer creates a ZIP archive when non data.table download formats are called (since adding =/zip= to the API endpoint URL already archives all generated files, this was redundant).
