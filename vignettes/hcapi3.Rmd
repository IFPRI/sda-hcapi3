...
This R package cannot be installed locally. Instead we provide access to all methods through a RESTful API (credits to (http://github.com/jeroenooms/opencpu)[OpenCPU]). Method names are endpoints and arguments must be converted from R syntax to simple JSON objects, `foo(arg1=c("val1", "val2"))` must be passed to `http://harvestchoice.org/tools/hcapi/R/foo` using `{"arg1" : ["val1", "val2"]}`.
