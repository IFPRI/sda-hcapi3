...
Other possible formats include `csv`, `json` for GeoJSON, and raster formats `grd`, `tif` and `asc` for R raster, GeoTiff, and ESRI ASCII raster respectively. Note that as-is these requests do not return actual data, but only a JSON response with (if the response is succesful) a pointer to a ZIP file. To download an actual dataset or raster layer you need to add `/zip` to the URL endpoint.
